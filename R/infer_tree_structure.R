...
#' @param kmeans.seed the seed for kmeans clustering.
#' @param clusters a named vector of cluster labels for the cells. If NULL (default), then clustering will be performed by kmeans.
#' @examples
#' data(hca_bm_pca)
#' data(hca_bm_saver)
  ...
  function(pca,
    ...
    if (is.null(clusters)) {
      ...
      if (is.null(names(clusters))) {
        stop("The 'clusters' vector must be a named vector with cell IDs as names.")
      }
      if (!all(rownames(pca) %in% names(clusters))) {
        stop("All rownames of 'pca' must be present in the names of 'clusters'.")
      }
      clu <- clusters[rownames(pca)] # ensure correct ordering
    }
    # -------------------------------
